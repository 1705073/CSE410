...
void drawWheel(double radius)
{
    ...
    for(i=1; i<segments; i++)
    {
        ...
        glBegin(GL_QUADS);
        {
            glVertex3f(points[i].x, -10, points[i].z);
            glVertex3f(points[i+1].x, -10, points[i+1].z);
            glVertex3f(points[i+1].x, 10, points[i+1].z);
            glVertex3f(points[i].x, 10, points[i].z);
        }glEnd();
    }
...
void move_forward()
{
    origin.x += (2.0/360.0) * 2 * pi * wheel_radius * cos(z_angle * pi / 180.0);
    origin.y += (2.0/360.0) * 2 * pi * wheel_radius * sin(z_angle * pi / 180.0);
    y_angle += 2.0;
}
